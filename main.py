...
# Режим ответа админа: admin_id -> user_id
admin_reply_mode = {}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Привет! Это анонимный бот. Отправляйте свои сообщения."
    )
    ...
    if data == "menu_blocked":
        ...
        for user_id in blocked_users:
            ...
            buttons.append(
                [
                    InlineKeyboardButton(
                        f"Разблокировать {user_id}", callback_data=f"unblock_{user_id}"
                    )
                ]
            )
        ...
        for m in list(message_history)[-N:]:
            lines.append(
                f"ID: {m['user_id']}, Username: @{m['username']}, Время: {m['time']}\nТекст: {m['text']}"
            )
        await query.edit_message_text("\n\n".join(lines), reply_markup=markup)

    elif data == "menu_broadcastall":
        await query.answer()
        keyboard = [[InlineKeyboardButton("Назад", callback_data="menu_main")]]
        markup = InlineKeyboardMarkup(keyboard)
...
async def broadcastall_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text:
        await update.message.reply_text("Используйте: /broadcastall <текст>")
        return
...
async def reply_user_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if admin_id in ADMINS:
        ...
        await query.edit_message_text(text=f"Введите ответ для пользователя {user_id}:")
    else:
